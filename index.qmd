...
#TOC::before {
  ...
  height:200px;
  width: 200px;
  background-image: url('./assets/PNG/graphical-abstract-medina.png');
...
The working example is a 1 ha clip from USGS 3DEP tile **10SGH1587** (Quality Level 1, ~78 pts/m², UTM 10N / NAVD88 Geoid18) over the Carr / Hirz / Delta Fires burn scar in Shasta County, California. The full tile is distributed through the [USGS Staged Elevation FTP directory](https://rockyweb.usgs.gov/vdelivery/Datasets/Staged/Elevation/LPC/Projects/CA_CarrHirzDeltaFires_2019_B19/CA_CarrHirzDeltaFires_2_2019/); the data validation report accompanies the download. A single 100 m × 100 m clip, centred on the tile's tallest-canopy cell, carries every chunk below one million points and drives all four chapters end-to-end.

The workflow uses R with `lidR` [@roussel2019lidr; @roussel2020] and `ForestTools` [@ForestTools]. Code examples are designed for operational use: large datasets, tile processing, and edge effects. Spatial operations lean on `sf` [@pebesma2021package; @pebesma2018simple; @pebesma2023spatial] and `terra` [@hijmans2023terra]. All dependencies are declared in the environment-setup chunk below.
...
![](./assets/PNG/graphical-abstract-medina.png){fig-align="center" width="95%"}
